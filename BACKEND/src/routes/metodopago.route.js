...
router.put(`/metodopagos/:id`, async (req, res) => {
    try {
        const {id} = req.params;
        const metodopago = await MetodoPago.update(req.body, {where: {id_metodoPago: id}});
...
router.delete(`/metodopagos/:id`, async (req, res) => {
    try {
        const {id} = req.params;
        await MetodoPago.destroy({where: {id_metodoPago: id}});
...
router.get(`/metodopagos/usuario/:id`, async (req, res) => {
    try {
        const {id} = req.params;
        const metodopagos = await MetodoPago.findAll({where: {id_usuario: id}});
        res.status(200).json(metodopagos);
    }catch (error) {
